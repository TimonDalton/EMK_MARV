...
Init:
    ...
    ; EUSART1 setup — TX on RC6, 9600 baud @ 4 MHz
    ; SPBRG1 = 25  ->  4000000 / (16 * (25+1)) = 9615 baud (~0.16% error)
    BCF     TRISC, 6, a         ; RC6 = output (TX1)
    MOVLW   0x19
    MOVWF   SPBRG1, a
    MOVLW   00100100B           ; TXEN=1, BRGH=1, SYNC=0
    MOVWF   TXSTA1, a
    MOVLW   10000000B           ; SPEN=1
    MOVWF   RCSTA1, a
...
Main:
    ...
    MOVLW   0xAA
    call    UART_TX
    MOVF    sensor_L_strobe_R, W, a
    call    UART_TX
    MOVF    sensor_L_strobe_G, W, a
    call    UART_TX
    MOVF    sensor_L_strobe_B, W, a
    call    UART_TX
    MOVF    sensor_C_strobe_R, W, a
    call    UART_TX
    MOVF    sensor_C_strobe_G, W, a
    call    UART_TX
    MOVF    sensor_C_strobe_B, W, a
    call    UART_TX
    MOVF    sensor_R_strobe_R, W, a
    call    UART_TX
    MOVF    sensor_R_strobe_G, W, a
    call    UART_TX
    MOVF    sensor_R_strobe_B, W, a
    call    UART_TX

    GOTO    Main

; ============================================================
; UART_TX: transmit W over EUSART1, blocks until TXREG is ready
UART_TX:
    BTFSS   PIR1, 4, a          ; TX1IF (PIR1<4>) = 1 when TXREG empty
    BRA     UART_TX
    MOVWF   TXREG1, a
    RETURN
